...
/* Walkabout ships as its own bundle (see src/vendor/walkabout/entry.js) so its
   vendored esprima does not weigh down every page load. Fetch it on demand. */
var walkaboutLoading = null;
function loadWalkabout() {
  if (window.TogetherJSWalkabout) {
    return Promise.resolve(window.TogetherJSWalkabout);
  }
  if (walkaboutLoading) {
    return walkaboutLoading;
  }
  walkaboutLoading = new Promise(function (resolve, reject) {
    var script = document.createElement("script");
    script.src = TogetherJS.baseUrl + "/walkabout.js";
    script.onload = function () {
      if (window.TogetherJSWalkabout) {
        resolve(window.TogetherJSWalkabout);
      } else {
        reject(new Error("walkabout.js loaded but did not register itself"));
      }
    };
    script.onerror = function () {
      walkaboutLoading = null;
      reject(new Error("could not fetch " + script.src));
    };
...
var commands = {
  ...
  command_test: function (args) {
    if (! Walkabout) {
      loadWalkabout().then((function (WalkaboutModule) {
        Walkabout = WalkaboutModule;
        this.command_test(args);
      }).bind(this), function (e) {
        ui.chat.system({text: "Could not load walkabout.js: " + e.message});
      });
      return;
    }
