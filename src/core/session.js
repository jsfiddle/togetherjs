 ...
 * URLs
 */
/* Config is read lazily throughout this module: it is evaluated as soon as the
   bundle loads, which is before the host page's configuration is applied. */
function includeHashInUrl() {
  return TogetherJS.config.close("includeHashInUrl");
}
...
var _ignoreMessages = null;
function ignoreMessage(type) {
  if (_ignoreMessages === null) {
    var configured = TogetherJS.config.get("ignoreMessages");
    if (configured === true) {
      DEBUG = false;
      _ignoreMessages = [];
      return true;
    }
    _ignoreMessages = configured || [];
  }
  return _ignoreMessages.indexOf(type) != -1;
}
...
function openChannel() {
  ...
  c.onmessage = function (msg) {
    ...
    if (DEBUG && ! ignoreMessage(msg.type)) {
      console.info("In:", msg);
    }
    if (! peers) {
...
function processFirstHello(msg) {
  if (! msg.sameUrl) {
    ...
    need("ui").showUrlChangeMessage(msg.peer, url);
    location.href = url;
  }
}
...
/* Feature modules used to be injected at runtime by RequireJS. They are all
   part of the bundle now (see ../index.js), so by the time start() runs they
   have registered their session handlers already. */

function getRoomName(prefix, maxSize) {
  var findRoom = TogetherJS.config.get("hubBase").replace(/\/*$/, "") + "/findroom";
  var url = new URL(findRoom);
  url.searchParams.set("prefix", prefix);
  url.searchParams.set("max", maxSize);
  return fetch(url, {headers: {Accept: "application/json"}}).then(function (resp) {
    if (! resp.ok) {
      throw new Error("findroom failed: " + resp.status + " " + resp.statusText);
    }
    return resp.json();
  }).then(function (body) {
    return body.name;
...
session.start = function () {
  initStartTarget();
  // Deferreds swallow exceptions thrown inside .then() callbacks, so without
  // this an error anywhere in the start chain just leaves TogetherJS silently
  // half-started. Surface it instead.
  function startupFailed(error) {
    console.error("TogetherJS failed to start:", error);
